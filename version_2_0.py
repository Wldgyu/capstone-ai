...
# ---------------------------------------------------------
async def final_total_process(input_data, input_type="image"):
    # [1단계 실행]
    ...
    for item in parsed_food_list:
        ...
        if match: final_nutrition_json.append(json.loads(match.group()))
...
# ---------------------------------------------------------
@app.post("/analyze")
async def handle_request(file: Optional[UploadFile] = File(None), text: Optional[str] = Form(None)):
    if file:
        content = await file.read()
        data = await final_total_process(content, "image")
        return {"status": "success", "data": data}
    elif text:
        data = await final_total_process(text, "text")
        return {"status": "success", "data": data}
